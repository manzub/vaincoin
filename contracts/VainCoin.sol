...
// demo token simplified
contract VainCoin is IERC20{
    using SafeMath for uint256;
    using Address for address;

    mapping (address => uint256) private _rOwned;//the balance of an address in reflected space
    mapping (address => uint256) private _tOwned;//the balance of an address in t-space
    mapping (address => mapping (address => uint256)) private _allowances;

    mapping (address => bool) private _isExcludedFromFee;

    // excluded addresses from reward
    mapping (address => bool) private _isExcluded;

    // also total supply of tokens
    uint256 private _tTotal = 200000 * 10**8; 
    // maximum possible supply of tokens
    uint256 private constant MAX = 20000000 *10**8;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    string private _name = "VainWorld";
    string private _symbol = "Vaincoin";
    uint8 private _decimals = 8;
    
    uint256 public _taxFee = 5;

    constructor() {
      _rOwned[msg.sender] = _rTotal; // e.g 100 in t-space = 100k in r-space all calculations are done in r-space

      // excluded contract creator from reward
      _isExcludedFromFee[msg.sender] = true;
      // excluded contract creator or zero address from reward
    ...
    function _reflectFee(uint256 rFee) private {
        _rTotal = _rTotal.sub(rFee); //we substract the fee charged in the tx (in R space) 100k - 500 = 99500
    }
    ...
    function _transfer(
        address from, //account 1
        address to, //account 2
        uint256 amount //10 with 1 decimal = 100
    ) private {
        ...
        //we work in R space, rAmount and rTransferAmount are decreasing but that does not mean they have less fees,
